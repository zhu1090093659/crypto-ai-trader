...
def safe_json_parse(json_str):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"JSON解析失败，原始内容: {json_str[:200]}")
...
def validate_and_correct_leverage(signal_data: Dict, config: Dict) -> Dict:
    ...
    # 如果没有提供杠杆值，使用默认值
    if leverage is None:
        logger.info(f"[{config['display']}] AI未返回杠杆值，使用默认值 {leverage_default}x")
    ...
    try:
        ...
        logger.warning(f"[{config['display']}] 杠杆值格式错误: {leverage}，使用默认值 {leverage_default}x")
        signal_data["leverage"] = leverage_default
        return signal_data
    ...
    if leverage < leverage_min or leverage > leverage_max:
        ...
        logger.info(f"[{config['display']}] 杠杆值 {original_leverage}x 超出配置范围 [{leverage_min}-{leverage_max}]，已修正为 {leverage}x")
        signal_data["leverage"] = leverage
    else:
        logger.info(f"[{config['display']}] ✓ 杠杆值 {leverage}x 在有效范围内")
        signal_data["leverage"] = leverage
...
def analyze_with_llm(symbol: str, price_data: Dict, config: Dict) -> Dict:
    ...
    logger.info(f"[{config['display']}] 🔍 AI分析-获取余额: {available_balance:.2f} USDT")

    # 2) 资金管理：预计算仓位组合
    current_price = price_data["price"]
    max_usable_margin = available_balance * 0.8
    logger.info(f"[{config['display']}] 🔍 最大可用保证金: {max_usable_margin:.2f} USDT (80%)")
    ...
    for confidence in ["HIGH", "MEDIUM", "LOW"]:
        ...
        for lev in leverage_list:
            ...
            if confidence == "LOW" and lev == config["leverage_max"]:
                logger.debug(f"[{config['display']}] 检查组合: {confidence}信心 + {lev}倍杠杆")
                logger.debug(f"[{config['display']}]    需要数量: {adjusted_quantity:.6f} ETH ({adjusted_contracts:.3f}张)")
                logger.debug(f"[{config['display']}]    需要保证金: {adjusted_margin:.2f} USDT")
                logger.debug(f"[{config['display']}]    最小合约: {min_contracts:.3f}张, 满足: {meets_min}")
                logger.debug(f"[{config['display']}]    保证金充足: {meets_margin} (需要{adjusted_margin:.2f} <= 可用{max_usable_margin:.2f})")
                logger.debug(f"[{config['display']}]    最终判断: {meets_min and meets_margin}")
    ...
    if not can_trade:
        ...
        logger.warning(f"[{config['display']}] 余额不足：即使最大杠杆也无法满足最小交易量 {min_quantity} ({min_contracts_display:.3f} 张)")
        logger.info(f"[{config['display']}]  当前余额: {available_balance:.2f} USDT")
        logger.info(f"[{config['display']}]  建议充值至少: {(min_quantity * current_price / config['leverage_max']):.2f} USDT")
        ...
        logger.info(f"[{config['display']}]  跳过AI分析（余额不足），直接返回HOLD信号")
    ...
    if sentiment_data:
        ...
        logger.info(f"[{config['display']}] {sentiment_text}")
    else:
        if token != "BTC":
            logger.info(f"[{config['display']}] {token}情绪数据不可用，尝试使用BTC市场情绪...")
    ...
    try:
        logger.info(f"⏳ 正在调用{AI_PROVIDER.upper()} API ({AI_MODEL})...")
        ...
        if not response:
            logger.warning(f"{AI_PROVIDER.upper()}返回空响应")
        ...
        if isinstance(response, str):
            ...
            logger.warning(f"{AI_PROVIDER.upper()}返回格式异常: {type(response)}")
            logger.debug(f"响应内容: {str(response)[:200]}")
            web_data["ai_model_info"]["status"] = "error"
            web_data["ai_model_info"]["error_message"] = "响应格式异常"
            return create_fallback_signal(price_data)

        if not result:
            logger.warning(f"{AI_PROVIDER.upper()}返回空内容")
            return create_fallback_signal(price_data)

        logger.debug("=" * 60)
        logger.debug(f"{AI_PROVIDER.upper()}原始回复:")
        logger.debug(result)
        logger.debug("=" * 60)
        ...
        if start_idx != -1 and end_idx != 0:
            ...
            if signal_data is None:
                logger.warning("JSON解析失败，使用备用信号")
                signal_data = create_fallback_signal(price_data)
            else:
                logger.info(f"✓ 成功解析AI决策: {signal_data.get('signal')} - {signal_data.get('confidence')}")
        else:
            logger.warning("未找到JSON格式，使用备用信号")
            signal_data = create_fallback_signal(price_data)
        ...
        if not all(field in signal_data for field in required_fields):
            ...
            logger.warning(f"缺少必需字段: {missing}，使用备用信号")
            signal_data = create_fallback_signal(price_data)
        ...
        logger.info(f"[{config['display']}] 信号统计: {signal_data['signal']} (最近{total_signals}次中出现{signal_count}次)")

        if len(history) >= 3:
            last_three = [s["signal"] for s in history[-3:]]
            if len(set(last_three)) == 1:
                logger.info(f"[{config['display']}] 注意：连续3次{signal_data['signal']}信号")
        ...
        if len(history) >= 20:
            ...
            if low_ratio > 0.5:
                logger.warning(f"[{config['display']}] 信心度警告：最近20次中{low_ratio*100:.0f}%是LOW，模型可能过于保守")
                logger.info(f"[{config['display']}]    分布: HIGH={conf_counts['HIGH']} MED={conf_counts['MEDIUM']} LOW={conf_counts['LOW']}")
            elif high_ratio < 0.2:
                logger.info(f"[{config['display']}]  提示：最近20次中HIGH仅{high_ratio*100:.0f}%，可能错过高确定性机会")

        return signal_data

    except Exception as e:
        logger.exception(f"[{config['display']}] {AI_PROVIDER.upper()}分析失败: {e}")
        import traceback
